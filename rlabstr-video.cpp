...
*/
void writemaze(int time, int xpos, int ypos, int action=-1)
{
	stringstream filename;
	...
	char markers[] = ">v<^";
	char agentmarker = markers[action];
	...
	for(int y=0;y<ysize;y++)
	{
		for(int x=0;x<xsize;x++)
		{
			if(x==xpos && y==ypos)
			{
				mazefile << agentmarker;
				commonfile << agentmarker;
			}
			else
			{
				char mark = ' ';
...
void writemaze_tex(int time, int xpos, int ypos, int action, string suffix)
{
	stringstream filename;
	filename << "history_" << setfill('0') << setw(3) <<  time  << suffix << ".tex";

	ofstream mazefile(filename.str().c_str());

	string markers[] = {"$\\medtriangleright$","$\\medtriangledown$","$\\medtriangleleft$","$\\medtriangleup$","$\\circ$"};
	string agentmarker = markers[action];

//mazefile << "\\begin{table}[htbp]" << end;
	mazefile << "\
	...
	for(int y=0;y<ysize;y++)
	{
		...
		for(int x=0;x<xsize;x++)
		{
			...
			else
			{
				string mark = " $~~~$ ";
				if(maze[x][y]==1)
					mark=" \\cellcolor[gray]{0.0} ";
				else
				{
					if(maze_visited[x][y]!=-1)
						mark = "$~\\cdot~$" ;//visited cell
				}
		...
		mazefile << "\\\\" << endl << "\\hline" << endl;
	}
	mazefile << "\\end{tabular}" << endl;
	mazefile << "\\end{document}" << endl;
}
...
#include"params.h"
	...
	*/	
	...
	for(time_step=0;time_step<10000 && all_world_explored != true;time_step++)
	{
	      ...
	      writemaze_tex(time_step,xpos,ypos,action,"A");
...
//				include_threshold=10;
		...
		/**The action the agent wishes to execute -- result of reinforcement learning computation on the model of the world.*/
	      action = read_sat_solution(time_step);
		...
		*/
	      ...
	      writemaze(time_step,xpos,ypos,action);
	      writemaze_tex(time_step,xpos,ypos,action,"B");
